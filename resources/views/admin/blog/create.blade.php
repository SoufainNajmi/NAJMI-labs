...
<div class="max-w-2xl mx-auto space-y-6">
    <div>
        ...
        <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Create Blog Article</h1>
    </div>
    ...
    <div class="glass p-6 rounded-2xl">
        <form action="{{ route('admin.blog.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="number" name="reading_time" id="reading_time" required min="1" value="{{ old('reading_time', 5) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:ring-primary focus:border-primary text-slate-900 dark:text-white">
            ...
            <div>
                ...
                <textarea name="content" id="content" required rows="10" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:ring-primary focus:border-primary text-slate-900 dark:text-white font-sans">{{ old('content') }}</textarea>
